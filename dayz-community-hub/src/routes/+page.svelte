<script lang="ts">
  ...
  let tabs = $derived([
    ...
    { id: 'favorites' as TabId, label: 'Favorites', count: s.profile?.favorites.length ?? 0 },
    { id: 'history' as TabId, label: 'History', count: s.profile?.history.length ?? 0 },
    { id: 'mods' as TabId, label: 'Mods', count: s.installedMods.length },
    ...
    if (e.ctrlKey && !e.shiftKey && !e.altKey && !e.metaKey) {
      ...
      if (e.key === 'u' && !isInput) {
        ...
        if (staleModCount > 0) {
          ...
          s.setStatus('All mods are up to date', 'success');
        }
        return;
      }
      if (e.key === 'l' && !isInput) {
  ...
  onMount(() => {
    ...
    listen<string>('launch-done', ({ payload }) => {
      s.setStatus(`Launched: ${payload}`, 'success');
      loadProfile();
    }).then((fn) => cleanupFns.push(fn));

    listen<string>('launch-error', ({ payload }) => {
      s.setStatus(`Launch error: ${payload}`, 'error');
    }).then((fn) => cleanupFns.push(fn));

    listen('offline-mode-updated', () => {
      s.offlineLoading = false;
      s.offlineStatus = 'Offline mode updated successfully';
    ...
    listen<string>('offline-mode-error', ({ payload }) => {
      s.offlineLoading = false;
      s.offlineStatus = `Update failed: ${payload}`;
