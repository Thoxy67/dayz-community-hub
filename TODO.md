...
- [x] Add server info key for a2s in history and favorite tabs
- [x] Add the install/update timestamp of a mod (local mtime + mod author timestamp)
- [x] Show the full list of mods in details (scrollable with Ctrl+d/Ctrl+u)
- [x] Find a workaround if user is not logged in steamcmd on mod downloading / updating (detects "Cached credentials not found", kills process, shows re-login command)
- [x] in the server list add r button to refresh and get the server list again from api
