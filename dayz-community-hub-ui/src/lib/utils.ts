...
export type PingState = 'pending' | 'timeout' | 'success';

/**
 * Determine the current ping state.
 * @param ms - The ping value in milliseconds
...
export function pingColor(ms: number | null | undefined): string {
  if (ms === undefined || ms === null || isTimeout(ms)) return 'text-base-content/30';
...
export function pingDot(ms: number | null | undefined): string {
  if (ms === undefined || ms === null || isTimeout(ms)) return 'bg-base-content/30';
