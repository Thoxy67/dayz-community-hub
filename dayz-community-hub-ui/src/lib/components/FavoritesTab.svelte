<script lang="ts">
  ...
  async function doRefreshPlayers(fav: FavoriteDto) {
    ...
    await serverData.refreshPlayers(fav.ip, queryPort);
  }
  ...
  async function openDetail(fav: FavoriteDto) {
    ...
    try {
      ...
      const result = await serverData.refreshA2s(fav.ip, queryPort);
      if (result) {
        a2s = result;
      } else {
        a2sError = serverData.getA2sError(fav.ip, queryPort) ?? 'A2S query failed';
      }
    } catch (e) {
      a2sError = String(e);
    } finally {
  ...
  <div class="flex flex-col flex-1 overflow-hidden">
    {#if favorites.length === 0}
      <div class="flex flex-col items-center justify-center h-full gap-3 text-base-content/40">
        ...
        <span class="text-xs text-base-content/30">{m.fav_no_favorites_hint()}</span>
      ...
      <div class="overflow-auto flex-1" bind:this={vl.container} onscroll={handleScroll}>
        <table class="w-full text-xs" style="table-layout: fixed; border-collapse: collapse;">
          ...
          <tbody>
            ...
            {/if}
            {#each visibleFavorites as fav, vi}
              {@const fi = startIndex + vi}
              {@const server = findServer(fav)}
              {@const pk = pingKey(fav)}
              ...
              {@const queryPort = server ? server.query_port : fav.port}
              {@const playerData = serverData.getPlayers(fav.ip, queryPort)}
              {@const livePlayers = playerData.players}
              {@const loadingPlayers = serverData.isA2sLoading(fav.ip, queryPort)}
