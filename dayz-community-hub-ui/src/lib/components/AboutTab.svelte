<script lang="ts">
  ...
  // Lazy-load SuikaGame only when easter egg is triggered (saves ~83kB)
  ...
  async function onKeydown(e: KeyboardEvent) {
    if (e.key === KONAMI[konamiIndex]) {
      ...
      if (konamiIndex === KONAMI.length) {
        konamiIndex = 0;
        if (!eggOpen) {
          // Lazy-load SuikaGame only when needed
          if (!SuikaGame) {
            SuikaGame = (await import('$lib/components/SuikaGame.svelte')).default;
          }
          eggOpen = true;
        }
        return;
  ...
  let pingScanFavorites = $derived(s.profile?.ping_scan_favorites ?? true);
  let pingScanHistory = $derived(s.profile?.ping_scan_history ?? true);
  let pingScanServers = $derived(s.profile?.ping_scan_servers ?? true);

  let saveTimeout: ReturnType<typeof setTimeout> | undefined;
  function debouncedSavePingSettings(
    concurrency: number,
    timeoutAuto: number,
    timeoutManual: number,
    maxRetries: number,
    scanFavorites: boolean,
    scanHistory: boolean,
    scanServers: boolean,
  ) {
    clearTimeout(saveTimeout);
    saveTimeout = setTimeout(() => {
      savePingSettings(concurrency, timeoutAuto, timeoutManual, maxRetries, scanFavorites, scanHistory, scanServers);
    }, 300);
  }
...
{#if eggOpen && SuikaGame}
  {@const Game = SuikaGame}
  <Game
    onClose={() => {
      eggOpen = false;
    }}
  ...
  <div class="max-w-6xl mx-auto px-6 py-6">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-6 items-start">
      ...
      <div class="space-y-6">
        ...
        <!-- Tips -->
        <section class="stagger-item" class:visible={mounted} style="transition-delay: 180ms">
          <div class="flex items-center gap-2 mb-3">
            <Icon icon="ph:lightbulb" class="size-4 text-primary shrink-0" />
            <h2 class="text-xs font-semibold text-base-content/60 uppercase tracking-widest">{m.about_tips()}</h2>
          </div>
          <div class="rounded-xl border border-base-300/60 bg-base-200/40 divide-y divide-base-300/40">
            <!-- AuthPlayerLoginState tip -->
            <div class="px-4 py-3">
              <p class="text-xs font-semibold text-base-content/70 mb-2">{m.about_tip_auth_title()}</p>
              <div class="space-y-1.5">
                {#each [m.about_tip_auth_step1, m.about_tip_auth_step2, m.about_tip_auth_step3, m.about_tip_auth_step4] as stepFn, i}
            ...
            <!-- Performance tip -->
            <div class="px-4 py-3">
              <p class="text-xs font-semibold text-base-content/70 mb-1">{m.about_tip_perf_title()}</p>
              <p class="text-xs text-base-content/50 leading-relaxed">{m.about_tip_perf_desc()}</p>
            </div>
            <!-- Ping Settings -->
            <div class="px-4 py-3 space-y-4">
              <p class="text-xs font-semibold text-base-content/70 mb-2">{m.about_ping_settings()}</p>
              <!-- Concurrency slider -->
              <div class="space-y-1">
                <div class="flex items-center justify-between">
                  <label class="text-xs text-base-content/60" for="ping-concurrency">{m.about_ping_concurrency()}</label
                  >
                  <span class="text-xs font-mono text-primary">{pingConcurrency}</span>
                </div>
                <input
                  type="range"
                  id="ping-concurrency"
              ...
              <div class="space-y-1">
                <div class="flex items-center justify-between">
                  <label class="text-xs text-base-content/60" for="ping-timeout-auto"
                    >{m.about_ping_timeout_auto()}</label
                  >
                  <span class="text-xs font-mono text-primary">{(pingTimeoutAuto / 1000).toFixed(1)}s</span>
                </div>
                <input
                  type="range"
                  id="ping-timeout-auto"
                  min="1000"
                  max="5000"
                  step="500"
                  value={pingTimeoutAuto}
                  oninput={(e) =>
                    debouncedSavePingSettings(
                      pingConcurrency,
                      parseInt(e.currentTarget.value),
              ...
              <!-- Manual timeout slider -->
              <div class="space-y-1">
                <div class="flex items-center justify-between">
                  <label class="text-xs text-base-content/60" for="ping-timeout-manual"
                    >{m.about_ping_timeout_manual()}</label
                  >
                  <span class="text-xs font-mono text-primary">{(pingTimeoutManual / 1000).toFixed(0)}s</span>
                </div>
                <input
                  type="range"
                  id="ping-timeout-manual"
                  min="1000"
                  max="30000"
                  step="1000"
                  value={pingTimeoutManual}
                  oninput={(e) =>
                    debouncedSavePingSettings(
                      pingConcurrency,
                      pingTimeoutAuto,
              ...
              <div class="space-y-1">
                <div class="flex items-center justify-between">
                  <label class="text-xs text-base-content/60" for="ping-max-retries">{m.about_ping_max_retries()}</label
                  >
                  <span class="text-xs font-mono text-primary"
                    >{pingMaxRetries === 0 ? m.about_ping_retry_disabled() : pingMaxRetries}</span
                  >
                </div>
                <input
                  type="range"
              ...
              <!-- Ping scan scope checkboxes -->
              <div class="space-y-2 pt-2 border-t border-base-300/30">
                <p class="text-xs text-base-content/60">{m.about_ping_scan_scope()}</p>
                <div class="flex flex-wrap gap-4">
                  <label class="flex items-center gap-2 cursor-pointer select-none">
                    <input
                      type="checkbox"
                      class="checkbox checkbox-xs checkbox-primary"
                      checked={pingScanFavorites}
                      onchange={(e) =>
                        debouncedSavePingSettings(
                          pingConcurrency,
                          pingTimeoutAuto,
                    ...
                    />
                    <span class="text-xs text-base-content/60">{m.about_ping_scan_favorites()}</span>
                  </label>
                  <label class="flex items-center gap-2 cursor-pointer select-none">
                    <input
                      type="checkbox"
                      class="checkbox checkbox-xs checkbox-primary"
                      checked={pingScanHistory}
                      onchange={(e) =>
                        debouncedSavePingSettings(
                          pingConcurrency,
                          pingTimeoutAuto,
                    ...
                    />
                    <span class="text-xs text-base-content/60">{m.about_ping_scan_history()}</span>
                  </label>
                  <label class="flex items-center gap-2 cursor-pointer select-none">
                    <input
                      type="checkbox"
                      class="checkbox checkbox-xs checkbox-primary"
                      checked={pingScanServers}
                      onchange={(e) =>
                        debouncedSavePingSettings(
                          pingConcurrency,
                          pingTimeoutAuto,
                    ...
                    />
                    <span class="text-xs text-base-content/60">{m.about_ping_scan_servers()}</span>
                  </label>
                </div>
              </div>
            </div>
