<script lang="ts">
  ...
  import ModConfirmModal from './ModConfirmModal.svelte';
  ...
  type PendingOperation = {
    type: 'update_stale' | 'update_all' | 'update_selected' | 'install';
    mods: Array<{
      id: number;
      name: string;
    ...
    ids: number[]; // Original IDs for the operation
  };
  let pendingOperation = $state<PendingOperation | null>(null);

  function confirmOperation() {
    if (!pendingOperation) return;
    const { type, ids } = pendingOperation;
  ...
  /** Show confirmation for updating stale mods (exported for external calls) */
  export function triggerUpdateStale() {
    const staleMods = mods.filter((m) => m.update_available);
    if (staleMods.length === 0) return;
    pendingOperation = {
      type: 'update_stale',
      mods: staleMods.map((m) => ({
        id: m.id,
        name: m.name,
      ...
      ids: staleMods.map((m) => m.id),
    };
  }

  /** Show confirmation for updating all mods */
  ...
  /** Show confirmation for updating selected mods */
  function triggerUpdateSelected(ids: number[], onlyStale: boolean = false) {
    const targetMods = mods.filter((m) => ids.includes(m.id) && (!onlyStale || m.update_available));
    ...
    pendingOperation = {
      ...
      ids: targetMods.map((m) => m.id),
    };
  }

  /** Show confirmation for installing new mods */
  function triggerInstall(ids: number[]) {
  ...
  let visibleMods = $derived(sorted.slice(startIndex, endIndex));
  ...
  let selectedPercent = $derived(mods.length > 0 ? Math.round((selectedIds.size / mods.length) * 100) : 0);
  let modsWithUpdates = $derived(mods.filter((m) => m.update_available));

  function selectAllWithUpdates() {
    selectedIds = new Set(modsWithUpdates.map((m) => m.id));
  }
...
<div class="flex flex-col h-full overflow-hidden">
  ...
  {:else}
    <div class="overflow-auto flex-1" bind:this={scrollContainer} onscroll={handleScroll}>
      <table class="w-full text-xs" style="table-layout: fixed; border-collapse: collapse;" bind:this={tableRef}>
        ...
        <tbody>
          {#if offsetY > 0}
            <tr><td colspan={canCheckUpdates ? 8 : 7} class="p-0 border-0" style="height:{offsetY}px"></td></tr>
          {/if}
          {#each visibleMods as mod, vi}
          ...
          {#if totalHeight - endIndex * ROW_HEIGHT > 0}
            <tr
              ><td
                colspan={canCheckUpdates ? 8 : 7}
                class="p-0 border-0"
                style="height:{totalHeight - endIndex * ROW_HEIGHT}px"
              ></td></tr
            >
          {/if}
        </tbody>
  ...
  {#if selectedIds.size > 0}
    <div class="flex items-center gap-3 px-4 py-2.5 bg-base-200 border-t-2 border-primary/40 flex-shrink-0 text-xs">
      ...
      <div class="flex items-center gap-2 text-base-content/70">
        ...
        >
        <span class="text-base-content/30">({selectedPercent}%)</span>
        ...
        {/if}
        {#if modsWithUpdates.length > 0 && selectedStaleCount < modsWithUpdates.length}
          <button
            type="button"
            class="btn btn-ghost btn-xs gap-1 text-warning/70 hover:text-warning"
            onclick={selectAllWithUpdates}
            title={m.mods_select_all_updates_title()}
          >
            <Icon icon="ph:selection-all" class="size-3" />
            {m.mods_select_all_updates()}
          </button>
        {/if}
      </div>
      ...
      <div class="flex items-center gap-1.5 ml-auto">
        ...
        <button
          class="btn btn-ghost btn-xs gap-1.5"
          onclick={() => triggerUpdateSelected(selectedMods.map((mod) => mod.id))}
          disabled={loading}
  ...
  {#if pendingOperation}
    <ModConfirmModal
      mods={pendingOperation.mods}
      mode={pendingOperation.type === 'install' ? 'install' : 'update'}
