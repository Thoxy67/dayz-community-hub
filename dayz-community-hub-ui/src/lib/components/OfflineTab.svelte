...
<div class="flex flex-col h-full overflow-hidden bg-base-100">
  ...
  {:else}
    <div class="flex-1 overflow-y-auto p-2 space-y-1">
      {#each missions as mission}
        <div
          class="group w-full text-left rounded-lg px-3 py-2.5 transition-colors border
                   hover:bg-base-200/70 border-transparent hover:border-base-300/50"
        >
          <div class="flex items-center gap-2.5">
            <div
              class="size-8 rounded-md flex items-center justify-center flex-shrink-0 bg-base-200 text-base-content/35"
            >
            ...
            <div class="flex items-center gap-1 shrink-0">
              <button
                ...
                onclick={() => onOpenMissionDir(mission)}
              ...
              <button
                type="button"
                class="btn btn-primary btn-xs px-2 gap-1"
                onclick={() => onLaunch(mission)}
                title={m.offline_launch()}
              >
                <Icon icon="ph:play-fill" class="size-3" />
                {m.offline_launch()}
              </button>
            </div>
          </div>
  ...
  <div class="px-3 pb-2.5 pt-2 flex-shrink-0 flex gap-1.5">
    ...
    <button
      class="btn btn-ghost btn-xs gap-1 text-base-content/35 hover:text-base-content/60"
